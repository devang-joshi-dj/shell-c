...
bool is_prime_number(const unsigned long num) {
	...
	if (num < 2) return false; // checking if num is less than 2
		...
		unsigned long num_root = (unsigned long)sqrt(num); // casted into unsigned long as sqrt returns double
		for (unsigned long i = 3; i <= num_root; i += 2) {
			if (!(num % i)) {
				return false; // current_number is not prime
			}
		}
 ...
 */
bool is_even(const unsigned long num) {
	return num % 2 == 0;
}
...
bool is_palindrome(unsigned long num) {
	const unsigned long original_num = num;
 ...
 */
bool is_armstrong_number(unsigned long num, const size_t digit_count) {
	const unsigned long original_num = num;
	unsigned long sum = 0;
	...
	while (num != 0) { // reversing the number through loop
		int unit_digit = get_unit_digit(num);
		sum += (unsigned long)pow(unit_digit, digit_count); // casted into unsigned long as pow returns double
		num /= 10;
	}
 ...
 */
bool is_perfect_number(const unsigned long num) {
	unsigned long sum = 1; // 1 is always a proper divisor

	if (num <= 1) return false; // 1 is not a perfect number

	unsigned long traverse_upto = (unsigned long)sqrt(num);

	for (unsigned long i = 2; i <= traverse_upto; i++) {
		if (num % i == 0) { // checking for proper divisors of the number
			sum += i;

			unsigned long pair = num / i;
 ...
 */
bool is_harshad_number(const unsigned long num, const size_t digit_sum) {
	if (!num) return false;
